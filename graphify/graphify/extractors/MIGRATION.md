...
| json_config | yes |
| (config-driven core: python, js, java, c, cpp, csharp, kotlin, scala, php, lua, swift, groovy, vue, svelte, astro, xaml, groovy) | no â€” shared _extract_generic core, move as one batch |
...
move with it for free â€” leave them exactly where they are. Only add a
...
3. Check whether tests/ exercises your language's behavior (grep for
   ...
   step 3 below is the ENTIRE proof of preservation â€” include the
...
5. `uv run pytest -q` -> 0 failures, no test file changed except the registry
   test. If ImportError/NameError: a helper was misclassified â€” go to
   Helper classification.
6. One commit: `refactor(extract): move extract_<lang> to extractors/<lang>.py (verbatim)`.

## What NOT to do

- Do not rewire dispatch, add classes, or add lazy imports â€” mechanism layers
