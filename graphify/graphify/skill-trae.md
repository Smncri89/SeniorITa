...
> Call `Task` once per chunk â€” ALL in the same response so they run in parallel.
> Trae does NOT support PreToolUse hooks â€” AGENTS.md rules are the always-on mechanism instead.
...
```bash
PROJECT_ROOT=$(pwd)  # cwd â€” where Part C globs graphify-out/ (NOT .graphify_root/scan dir, #1392)
