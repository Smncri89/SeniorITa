...
### Step 0 â€” Constrained query expansion (REQUIRED before traversal)

graphify's `query` CLI matches nodes via case-folded substring + IDF â€” there is **no stemming, no synonyms, no cross-language match** inside the binary, and the inline fallback below matches the same way. If the user's question uses different language or different domain vocabulary than the graph's labels (user says "Ð¾Ð±Ñ€Ð°Ð±Ð¾Ñ‚Ñ‡Ð¸Ðº" / graph says "handler"; user says "authentication" / graph says "Guardian"), the literal matcher returns 0 hits and the answer collapses to noise.
...
2. Read `graphify-out/.vocab.txt`. Then for the user's question, select **up to 12 tokens from this exact list** that semantically match the query intent. Hard constraints:
   ...
   - If a query concept has no plausible token in the vocab, skip it â€” do not substitute a near-synonym from training memory.
   ...
   - Translate cross-language: Russian "Ð°ÑƒÑ‚ÐµÐ½Ñ‚Ð¸Ñ„Ð¸ÐºÐ°Ñ†Ð¸Ñ" â†’ look for `auth`, `credential`, `token`, `security` IFF present in vocab.
...
```
If the list is empty, say so plainly and stop â€” do not proceed to traversal.

### Step 1 â€” Traversal

Build the **expanded query string** by joining the selected tokens with spaces. Use this string as `QUESTION` below â€” NOT the original user question. (The original question is preserved only for `save-result` at the end.)
...
**Work memory (self-improving loop).** Add an `--outcome` so future sessions learn from this one â€” append `--outcome useful|dead_end|corrected` to the `save-result` command (and `--correction "the right answer"` when correcting):

- `useful` â€” the cited nodes answered the question well (they become *preferred sources*).
- `dead_end` â€” the question/path led nowhere; don't re-derive it next time.
- `corrected` â€” the saved answer was wrong; `--correction` records what was right.
