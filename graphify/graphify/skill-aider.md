...
- A reading list (papers + tweets + notes â†’ one navigable graph)
...
# 1. uv tool installs â€” most reliable on modern Mac/Linux
...
> **Aider platform:** Multi-agent support is still early on Aider. Extraction runs sequentially â€” you read and extract each file yourself. This is slower than parallel platforms but fully reliable.

Print: `"Semantic extraction: N files (sequential â€” Aider)"`
...
If `code_only` is False (any changed file is a doc/paper/image): run the full Steps 3Aâ€“3C pipeline as normal.
...
Skip Steps 1â€“3. Load the existing graph from `graphify-out/graph.json` and re-run clustering:
...
Then run Steps 5â€“9 as normal (label communities, generate viz, benchmark, clean up, report).
...
- Images (.png/.jpg/.webp) â†’ downloaded, vision extraction runs on next build
