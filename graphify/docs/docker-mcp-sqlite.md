...
This document is *not* required to use graphify â€” it lives here as a known-good
...
# â†’ 15 tools (was 9 before adding SQLite)
```
...
inside containers:
```
mcp-sqlite (named volume) â†’ /mcp/db.sqlite
```
...
## Troubleshooting
- **`starting client: calling "initialize": EOF`** â€” the requested server
...
- **`cannot use --enable-all-servers with --servers flag`** â€” these gateway
  args are mutually exclusive; pick one.
- **No new tools appear in `docker mcp tools count` after install** â€” the
  gateway may be running with `dynamic-tools` enabled, exposing only meta-tools
  (`mcp-add`, `mcp-find`, â€¦) until a profile is activated mid-session. Either
