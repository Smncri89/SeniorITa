...
Nothing â€” the token is one-shot. Once consumed (by the hook allowing an operation), the token file is deleted. If no matching operation runs, the token stays there indefinitely; you can also `rm` it manually. The token file contains no secrets.

**Q: Can the agent create the unlock token itself?**
Yes, technically â€” the token is just a file. This is a known limitation documented in `PROTOCOL.md` Â§9. Mitigations: RULE-017 makes the agent's own token creation itself a policy modification (audit-logged); OS-level ACL denying agent-user write to `~/.claude/.policy-unlock` (roadmap).
...
It's a *deliberate trade-off* documented in `SECURITY.md`. Rationale: a parser-fragile hook that fail-closes on any hiccup would render Claude Code unusable on the first protocol/harness mismatch. The alternative â€” reviewing every parse failure via `audit.jsonl` and hardening incrementally â€” is preferable.

**Q: How is this different from just prompting Claude to be careful?**
Two things: (1) the hooks are *structural* â€” the model cannot argue past them, they run in the harness before the tool call executes. (2) the audit log is *persistent* â€” a policy violation leaves evidence outside the chat.
...
**Q: We're a big org â€” how does this scale to 200 devs?**
...
Partially â€” see `PROTOCOL.md` Â§11 question 6. The mitigation is *cross-model triangulation* (Claude proposed, Gemini + ChatGPT reviewed) plus human final approval on every version bump. The convergence-adoption rule (require â‰¥2 independent reviewers to flag an issue) reduces single-model bias.
