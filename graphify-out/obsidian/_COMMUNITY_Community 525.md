---
type: community
cohesion: 1.00
members: 2
---

# Community 525

**Cohesion:** 1.00 - tightly connected
**Members:** 2 nodes

## Members
- [[Two labels for one ID from one file the loser's label is discarded, which is]] - rationale - graphify/tests/test_dedup.py
- [[test_same_file_relabel_is_noted()]] - code - graphify/tests/test_dedup.py

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Community_525
SORT file.name ASC
```

## Connections to other communities
- 1 edge to [[_COMMUNITY_Community 87]]
- 1 edge to [[_COMMUNITY_Community 69]]

## Top bridge nodes
- [[test_same_file_relabel_is_noted()]] - degree 3, connects to 2 communities