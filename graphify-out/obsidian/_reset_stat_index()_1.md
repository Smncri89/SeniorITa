---
source_file: "graphify/tests/test_stat_index_portability.py"
type: "code"
community: "Community 130"
location: "L26"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_130
---

# _reset_stat_index()

## Connections
- [[The stat-index locationanchor are chosen once per process via module     globa]] - `rationale_for` [EXTRACTED]
- [[test_cache_hits_survive_corpus_move()]] - `calls` [EXTRACTED]
- [[test_deleted_entries_are_pruned_on_flush()]] - `calls` [EXTRACTED]
- [[test_legacy_absolute_index_migrates_gracefully()]] - `calls` [EXTRACTED]
- [[test_out_of_root_key_round_trips_absolute()]] - `calls` [EXTRACTED]
- [[test_relative_key_wins_over_colliding_legacy_absolute()]] - `calls` [EXTRACTED]
- [[test_semantic_cache_normalizes_absolute_source_file()]] - `calls` [EXTRACTED]
- [[test_semantic_cache_normalizes_backslash_poisoned_source_file()]] - `calls` [EXTRACTED]
- [[test_stat_index_portability.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_130